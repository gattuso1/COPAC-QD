module Integrals_Dexmax


contains

real*8 function Dex_MCMCoff(A1,B1,kin1,kout1,A2,B2,kin2,kout2,A3,B3,kin3,kout3,A4,B4,kin4,kout4,a,b,l,flag)
      ...
      real(dp) :: f1, f2, f3, f4, f5, f6, f7, f8, f9, PI, interval, f, eps, eps0, elec, &
                f1mc, f2mc, f3mc, f1mcmc, f2mcmc, f3mcmc, add, maxrad, vola, volb, volouta, voloutb, volout, start, finish, &
                epsR, epsinf, epsinfa, epsinfb, eps1, eps2, side
      real(dp) :: A1,B1,kin1,kout1,A2,B2,kin2,kout2,A3,B3,kin3,kout3,A4,B4,kin4,kout4,a, b, l
      real(dp), allocatable:: r1(:,:) , r2(:,:) , ra(:), rb(:), rc(:), rd(:), &
                              r1Anorm(:), r2Anorm(:), rnorm(:), RAB(:), r1Bnorm(:), r2Bnorm(:)

      include '../inputs/GetVarex.f90'
      ...
      if ((r1Anorm(i) .le. a) .and. (r1Bnorm(i) .gt. b)) then !inA
         ...
         do j=1,m
               ...
               exit
                  ...
                  f1 = f1 + wf(inA_inB,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) ra(i),rb(j),rc(i),rd(j), &
                  !wf(inA_inB,kin1,kout1,ra(i),kin2,kout2,rb(j),kin3,kout3,rc(i),kin4,kout4,rd(j))
                  j1=j1+1
                  ...
                  f2 = f2 + wf(inA_inA,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) ra(i),rb(j),rc(i),rd(j), &
                  !wf(inA_inB,kin1,kout1,ra(i),kin2,kout2,rb(j),kin3,kout3,rc(i),kin4,kout4,rd(j))
                  j2=j2+1
                  ...
                  if ((r2Anorm(j)-a) .lt. (r2Bnorm(j)-b)) then
                     ...
                     !write(6,*) "A", r2Anorm(j)-a, r2Bnorm(j)-b, eps2
                     else
                     eps2 = epsout + (epsinfb-epsout)*((PI/2) - atan(slope*(r2Bnorm(j)-b)))/PI
                     !write(6,*) "B", r2Anorm(j)-a, r2Bnorm(j)-b, eps2
                  endif
                  f3 = f3 + wf(inA_out,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !wf(inA_out,kin1,kout1,ra(i),kin2,kout2,rb(j),kin3,kout3,rc(i),kin4,kout4,rd(j))
                  !write(6,*) j3, f3
                  j3=j3+1
              endif
         enddo
         ...
         do j=1,m
               ...
               exit
                  ...
                  f4 = f4 + wf(inB_inB,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) 'BB', j4
                  j4=j4+1
                  ...
                  eps2 = epsout + (epsinfa-epsout)*((PI/2) - atan(slope*(r2Anorm(j)-a)))/PI
                  f5 = f5 + wf(inB_inA,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) 'BA', j5
                  j5=j5+1
                  ...
                  f6 = f6 + wf(inB_out,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) r1Bnorm(i), eps1, eps2
                  j6=j6+1
              endif
...
!write(6,*) f4, f5, f6
         ...
         do j=1,m
               ...
               exit
                  ...
                  eps2 = epsout + (epsinfb-epsout)*((PI/2) - atan(slope*(r2Bnorm(j)-b)))/PI
                  f7 = f7 + wf(out_inB,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) 'OB', j7
                  j7=j7+1
                  ...
                  f8 = f8 + wf(out_inA,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) 'OA', j8
                  j8=j8+1
                  ...
                  f9 = f9 + wf(out_out,kin1,kout1,ra(i),kin2,kout2,rb(i),kin3,kout3,rc(j),kin4,kout4,rd(j)) /&
                            (rnorm(j)*sqrt(eps1*eps2))
                  !write(6,*) 'OO', wf(out_out,kin1,kout1,ra(i),kin2,kout2,rb(j),kin3,kout3,rc(i),kin4,kout4,rd(j))
                  j9=j9+1
              endif
...
!write(6,*) (elec/(64*PI**3*eps0))*(f1mcmc), (elec/(64*PI**3*eps0))*(f2mcmc), (elec/(64*PI**3*eps0))*(f3mcmc)
...
!enddo
      ...
      Dex_MCMCoff=(elec/(64*PI**3*eps0))*(f1mcmc+f2mcmc+f3mcmc)

end function Dex_MCMCoff
